...
function unDB<T extends Omit<DatabaseDan<string>, 'createdAt' | 'updatedAt'> & Partial<Pick<DatabaseDan<string>, 'createdAt' | 'updatedAt'>>>(val: T): T & { _db: boolean } {
  val = validateUsecase(val)
  ;(val as any)._db = false
  val.id = ''
  return val as T & { _db: boolean }
}
