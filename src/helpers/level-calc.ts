...
export function getRequiredScoreForLevel(level: number) {
  if (level <= 100) {
    if (level > 1)
      return BigInt(Math.floor(5000 / 3 * (4 * level ** 3 - 3 * level ** 2 - level) + Math.floor(1.25 * 1.8 ** (level - 60))))

    return BigInt(1)
  }
  return BigInt('26931190829') + BigInt(100000000000) * BigInt(level) - BigInt(100)
}
