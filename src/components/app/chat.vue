...
defineExpose({
  selectIdx: onIdxSelected,
})
</script>

<template>
  <div class="flex flex-col md:flex-row">
    <ul class="overflow-x-auto rounded-none menu menu-horizontal md:menu-vertical bg-base-200 lg:rounded-l-lg flex-nowrap shrink-0">
    ...
    <div v-if="idxMsg && cur" class="flex flex-col overflow-y-auto border-l auto-border-color grow">
      <div class="flex gap-2 p-4 border-b auto-border-color bg-base-50">
      ...
      <div ref="messages" class="p-4 overflow-y-scroll border-b auto-border-color bg-base-100 grow">
    ...
    >
      <div class="relative w-full h-full max-w-screen-xl p-4 mx-auto bg-base-200 rounded-2xl md:min-w-1/2">
        <div
        ...
        >
          <div class="flex items-baseline justify-between">
            <div class="mb-2 card-title shrink-0">
              User
            </div>
          ...
          <div class="relative order-2 mt-auto shrink-0 md:mt-0 md:mb-4 md:order-1">
            ...
            <input
              ...
              name="keyword"
              type="search"
              class="block w-full p-4 text-sm text-gray-900 border rounded-lg auto-border-color ps-10 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Search User"
              @input="(v: Event) => onInputHandle((v as any).target.value)"
            >
          </div>
          <div class="grid order-1 gap-4 overflow-y-auto md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 rounded-xl">
            <button
            ...
            >
              <div class="flex items-center justify-center gap-2 pointer-events-none md:justify-start face">
        ...
        <div
          class="absolute inset-0 transition-opacity opacity-0 pointer-events-none"
          :class="{
            'opacity-100 pointer-events-auto': modalState === ModalState.Loading,
          }"
        >
          <div class="flex items-center justify-center w-full h-full">
            <span class="w-10 h-10 loading" />
          </div>
        </div>
      </div>
    </t-modal>
...
.auto-border-color {
  ...
  @apply border-gbase-300 dark:border-gbase-700
}
</style>
