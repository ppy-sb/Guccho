...
<template>
  <div class="roller-item" :style="{ width: `${width}px` }">
    <div v-if="isEnd" class="roller-item__wrapper" :class="{ 'roller-item__wrapper--short': mode === RollerItemMode.SHORT }">
      <div class="roller-item__wrapper__list">
        <div v-for="item of shortCharSet" ref="itemElements" :key="item" class="roller-item__wrapper__list__item" :class="{ 'roller-item__wrapper__list__item--target': item === char }">
          {{ item }}
        </div>
      </div>
    </div>
    <div v-else class="roller-item__wrapper" :class="{ 'roller-item__wrapper--short': mode === RollerItemMode.SHORT }">
      <div class="roller-item__wrapper__list top-[25%]" :style="{ transform: `translateY(${top})`, transition: `transform ${duration}ms` }">
        <div v-for="item of charSet" ref="itemElements" :key="item" class="roller-item__wrapper__list__item" :class="{ 'roller-item__wrapper__list__item--target': item === char }">
          {{ item }}
        </div>
      </div>
    </div>
